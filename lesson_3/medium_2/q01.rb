...
#initiall variables set; going into the block variabled stay the same with same object_id; when setting a new value to original variable the object_id changes; when setting new variables (*_inner) to original variable (*_outer) Ruby keeps the same object_id; all chnges inside the block stay outside the block but all variables defined insiade the block do not get to leave outside the blockand lose meaning.
